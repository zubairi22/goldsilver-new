...
const handleAdd = () => {
    addForm.post(route('outlet.categories.store'), {
        preserveScroll: true,
...
const handleEdit = () => {
    editForm.patch(route('outlet.categories.update', editForm.id), {
        preserveScroll: true,
...
const handleDelete = () => {
    router.delete(route('outlet.categories.destroy', deleteForm.id), {
        preserveScroll: true,
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="py-8 space-y-6">
            <Heading class="mx-4" title="Kategori" description="Kelola daftar kategori produk" />
            <div class="mx-auto max-w-8xl">
                <Card class="py-4 md:mx-4">
                    <CardContent>
                        <div class="flex flex-col justify-between md:flex-row mb-2">
                            <div class="mb-3 md:mb-0">
                                <Button @click="addModal = true">Tambah Kategori</Button>
                            </div>
                        </div>
                                ...
                                <TableBody>
                                    <TableRow v-for="cat in categories.data" :key="cat.id">
                                        ...
                                        <TableCell class="px-0.5">
                                            <EditButton @click="openEdit(cat)"/>
                                        </TableCell>
                                        <TableCell class="px-0.5">
                                            <DeleteButton @click="openDelete(cat)"/>
                                        </TableCell>
                                    </TableRow>
                                    <TableRow v-if="!categories.total">
                                        <TableCell colspan="3" class="text-center text-gray-500">Data tidak ditemukan.</TableCell>
                                    </TableRow>
                                </TableBody>
                            </Table>
                            <PageNav :data="categories" />
                        </div>
                    </CardContent>
                </Card>
            </div>
    ...
    <Dialog :open="addModal" @update:open="v => addModal = v">
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>Masukkan nama kategori produk.</DialogDescription>
            </DialogHeader>
            <div class="space-y-3">
                <div>
    ...
    <Dialog :open="editModal" @update:open="v => editModal = v">
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>Perbarui nama kategori produk.</DialogDescription>
            </DialogHeader>
            <div class="space-y-3">
                <div>
                    <label class="text-sm mb-1 block">Nama</label>
